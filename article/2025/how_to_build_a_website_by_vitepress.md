...
如果你对前端一窍不通，选择第一个然后就可以直接使用了，默认主题提供了许多功能，具体看官方文档。如果想要自己做一些简单的修改可以选第二个，继承默认主题，免得踩很多坑。如果是第三个需要完全自己来写。
...
然后你需要在网站目录下创建一个`public`文件夹，这个文件夹目的是存储图片或脚本等非`.md`静态资源，构建的时候会直接放到根目录中。（个人推测是为了保持根目录整洁的设计）
